...
export const ListItem: React.FC<ListItemProps> = ({ children, ordered = false, className = "" }) => (
  <li className={`mb-2 flex items-start ${className} ${ordered ? 'list-item-ordered' : 'list-item-unordered'}`}>
    {!ordered && (
    ...
    <span className="text-starlight-blue text-base leading-relaxed">{children}</span>
  </li>
);

export const SubListItem: React.FC<ListItemProps> = ({ children, ordered = false, className = "" }) => ( 
  <li className={`ml-5 mb-1 flex items-start ${className} ${ordered ? 'list-item-ordered' : 'list-item-unordered'}`}> 
     {!ordered && (
    ...
    <span className="text-starlight-blue/90 text-sm leading-relaxed">{children}</span> 
  </li>
);
