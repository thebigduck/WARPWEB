// --- File: src/components/documentation/ArmorSystemDoc.tsx ---
    ...
    └── ArmorSystem/
        ...
        └── Private/
            ...
            └── ArmorSystemComponent.cpp`}
                ...
                <p className="text-sm italic text-starlight-blue/70 mt-2">Note: <CodeBlock inline>ItemSystemTypes.h</CodeBlock>, <CodeBlock inline>DamageSystemTypes.h</CodeBlock>, and <CodeBlock inline>MyGameGameplayEffectContext.h</CodeBlock> are now owned by the <strong>DamageSystem</strong> plugin.</p>
            </div>
        )}
      </section>
        ...
        {expandedSections[sectionId('integration-points-sublink')] && (
            <div className="pl-6 pt-3 space-y-3">
                ...
                <ul className="list-none space-y-1 pl-0">
                 ...
                 <CodeBlock>{`ArmorComponent->ProcessDamageInteraction(FDamageContext& Context, FGameplayTag HitLimbTag, FName HitBoneName);`}</CodeBlock>
...
UPROPERTY(VisibleAnywhere) UArmorSystemComponent* ArmorComponent;`}</CodeBlock>
    ...
    ${q}DamageSystem${q}
                    ...
                    <ListItem ordered><strong>Equipping Logic</strong>
                        ...
                        <CodeBlock>{`ArmorComponent->Server_EquipArmor(ArchetypeData, DesiredQuality, SlotTag);`}</CodeBlock>
                        <p className="mt-1">On unequip:</p>
                        <CodeBlock>{`ArmorComponent->Server_UnequipArmor(SlotTag);`}</CodeBlock>
                    </ListItem>
                    <ListItem ordered><strong>ProcessDamageInteraction</strong>
                        ...
                        <CodeBlock>{`void UArmorSystemComponent::ProcessDamageInteraction(
    FDamageContext& Context,
    FGameplayTag HitLimbTag,
    FName HitBoneName
);`}</CodeBlock>
                        <p className="mt-1">Use <CodeBlock inline>Context.ProjectileMassKg</CodeBlock>, <CodeBlock inline>Context.ProjectileCaliberMm</CodeBlock>, <CodeBlock inline>Context.ProjectileRoundType</CodeBlock> with material factors to compute: Penetration chance, Ricochet probability, Spall energy. Update <CodeBlock inline>Context.CurrentDamageToApply</CodeBlock> and reduce <CodeBlock inline>CurrentDurability</CodeBlock>.</p>
                    </ListItem>
                </ol>
            </div>
        )}
